...
CREATE POLIBY "Allow authenticated users to read parking_lots" ON parking_lots FOR SELECT TO authenticated USING (true);
...
CREATE POLIBY "Allow superadmins to update parking_lots" ON parking_lots FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'superadmin'));
CREATE POLIBY "Allow superadmins to delete parking_lots" ON parking_lots FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'superadmin'));
...
CREATE POLIBY "Allow admins to update user_roles" ON user_roles FOR UPDATE TO authenticated USING (
...
CREATE POLIBY "Allow authenticated users to read parking_sessions" ON parking_sessions FOR SELECT TO authenticated USING (true);
CREATE POLIBY "Allow guards and admins to manage parking_sessions" ON parking_sessions FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'superadmin') OR
...
CREATE POLIBY "Allow guards and admins to manage vehicle_novelties" ON vehicle_novelties FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'superadmin') OR
...
CREATE POLIBY "Allow authenticated users to read parking_lot_sequences" ON parking_lot_sequences FOR SELECT TO authenticated USING (true);
...
CREATE POLIBY "Superadmins can update global app settings" ON global_app_settings FOR UPDATE USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'superadmin')
);
