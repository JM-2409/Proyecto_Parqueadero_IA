...
export default function AdminDashboard({
  ...
  const [adminCheckoutSession, setAdminCheckoutSession] = useState<any | null>(
    null,
  );
  ...
  const [parkingLotSubscriptionPlan, setParkingLotSubscriptionPlan] =
    useState("trial");
  const [parkingLotSubscriptionEndDate, setParkingLotSubscriptionEndDate] =
    useState("");
  ...
  const fetchSettings = async () => {
    ...
    if (settingsData) {
      settingsData.forEach((setting) => {
        ...
        if (setting.key === "private_spot_fields")
          setPrivateSpotFields(setting.value);
        if (setting.key === "capacity_settings")
          setCapacitySettings(setting.value);
        if (setting.key === "revenue_settings")
          setRevenueSettings(setting.value);
        if (setting.key === "guard_permissions")
          setGuardPermissions(setting.value);
        if (setting.key === "special_vehicles")
          setSpecialVehicles(setting.value);
    ...
    const { data: lotData } = await supabase
      .from("parking_lots")
      .select(
        "name, nit, address, phone, email, logo_url, subscription_plan, subscription_end_date",
      )
      .eq("id", parkingLotId)
      .single();
  ...
  const saveGuardPermissions = async () => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        {
          key: "guard_permissions",
          value: guardPermissions,
          parking_lot_id: parkingLotId,
        },
        { onConflict: "key,parking_lot_id" },
      );
  ...
  const handleCashClosing = async () => {
    if (
      !window.confirm(
        "¿Estás seguro de que deseas realizar el cierre de caja? Esto reiniciará el contador de ingresos acumulados a cero.",
      )
    )
      return;
  ...
  const handleBulkClear = () => {
    ...
    if (
      !confirm(
        `¿Estás seguro de que deseas vaciar la columna "${bulkClearColumn}" para TODOS los parqueaderos privados? Esta acción no se puede deshacer.`,
      )
    )
      return;
  ...
  const addEntryField = () => {
    ...
    setEntryFields([
      ...
      {
        id: newId,
        label: `Nuevo Campo ${entryFields.length + 1}`,
        required: false,
        enabled: true,
      },
    ]);
  ...
  const updateEntryField = (id: string, updates: any) => {
    setEntryFields(
      entryFields.map((f) => (f.id === id ? { ...f, ...updates } : f)),
    );
  };

  const addPrivateSpotField = () => {
    const newId = `campo_${Date.now()}`;
    setPrivateSpotFields([
      ...privateSpotFields,
      {
        id: newId,
        label: `Nuevo Campo ${privateSpotFields.length + 1}`,
        required: false,
        enabled: true,
      },
    ]);
  ...
  const updatePrivateSpotField = (id: string, updates: any) => {
    setPrivateSpotFields(
      privateSpotFields.map((f) => (f.id === id ? { ...f, ...updates } : f)),
    );
  };
  ...
  const savePrivateSpotFields = async () => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        {
          key: "private_spot_fields",
          value: privateSpotFields,
          parking_lot_id: parkingLotId,
        },
        { onConflict: "key,parking_lot_id" },
      );
  ...
  const fetchData = async () => {
    ...
    const lastClosing =
      revSettings?.value?.last_closing || new Date(0).toISOString();
    ...
    if (sessions) {
      ...
      sessions.forEach((s) => {
        ...
        if (
          s.status === "completed" &&
          s.exit_time &&
          new Date(s.exit_time) >= new Date(lastClosing)
        ) {
  ...
  const getRateTypeName = (type: string) => {
    switch (type) {
      ...
      case "day_night":
        return "Día y Noche";
      case "hour":
        return "Por Hora (Legado)";
      case "hour_minute":
        return "Hora + Minuto (Legado)";
      case "flat":
        return "Tarifa Plana (Legado)";
      case "daily":
        return "Por Día (Legado)";
      default:
        return type;
    }
  };
          ...
          <div className="relative">
            ...
            <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-full overflow-hidden flex items-center justify-center shadow-xl shrink-0 aspect-square bg-white">
              {!logoError ? (
                ...
                <Car className="w-8 h-8 text-indigo-600 group-hover:scale-110 transition-transform" />
              )}
            </div>
          </div>
                ...
                <tbody>
                  {privateSpots.length === 0 ? (
                    <tr>
                      <td
                        colSpan={
                          privateSpotFields.filter((f) => f.enabled).length + 1
                        }
                        ...
                        >
                          {privateSpotFields
                            ...
                            .map((field) => (
                              <td
                                key={field.id}
                                className={`px-4 py-3 ${field.id === "licensePlate" ? "font-mono text-slate-600" : field.id === "spotNumber" ? "font-medium text-slate-800" : "text-slate-600"}`}
                              >
                                {field.id === "vehicleType"
                                  ? spot[field.id] === "car"
                                    ? "Carro"
      ...
      {activeTab === "settings" && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
              ...
              {loadingSuperSettings ? (
                ...
                <div className="space-y-6 max-w-2xl">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    * El Nombre y el NIT solo pueden ser modificados por el
                    Super Administrador.
                  </p>
                      ...
                      <div className="font-semibold text-indigo-700 capitalize">
                        {parkingLotSubscriptionPlan === "trial"
                          ...
                          : parkingLotSubscriptionPlan === "monthly"
                            ? "Mensual"
                            : parkingLotSubscriptionPlan === "semi-annual"
                              ? "Semestral"
                              : parkingLotSubscriptionPlan === "annual"
                                ? "Anual"
                                : parkingLotSubscriptionPlan === "expired"
                                  ? "Expirado"
                                  : parkingLotSubscriptionPlan}
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-indigo-900 mb-1">
            ...
            <div className="p-6">
              <div className="space-y-6 max-w-3xl">
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  >
                    Habilitar límites de capacidad (No permitir ingresos si el
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="p-4 border border-slate-200 rounded-xl space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <label
                        htmlFor="allow_cars"
                        className="font-medium text-slate-700 flex items-center gap-2"
                      >
                  ...
                  {guardPermissions.show_history && (
                    <div className="ml-8 mt-4 pt-4 border-t border-slate-200">
                      ...
                      <p className="text-xs text-slate-500 mt-2">
                        Define cuántos días de historial pueden consultar los
                        guardas en su panel.
                      </p>
                    </div>
                  )}
                ...
                <div className="p-6 border border-slate-200 rounded-xl bg-white flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      Último cierre:{" "}
                      {format(
                        new Date(revenueSettings.last_closing),
                        "dd/MM/yyyy h:mm a",
                      )}
                    </p>
                    <p className="text-sm text-slate-500">
                      Ingresos acumulados desde el último cierre:{" "}
            ...
            <div className="p-6">
              ...
              {specialVehicles.length > 0 ? (
                      ...
                      {specialVehicles.map((v) => (
                        <tr key={v.id}>
                          ...
                          <td className="py-3">
                            {v.rate_type === "monthly"
                              ? "Mensualidad"
                              : v.rate_type === "fixed_daily"
                                ? "Fijo por Día"
                                : v.rate_type === "day_only"
                                  ? "Solo Día"
                                  : "Solo Noche"}
                          </td>
                ...
                <p className="text-center text-slate-500 py-4">
                  No hay vehículos con tarifas especiales configurados.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] shadow-xl border border-white overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-slate-600 mb-6">
                Configura los campos que se deben llenar al registrar un
                parqueadero privado.
              </p>
              ...
              {loadingSettings ? (
                ...
                <div className="space-y-4">
                  {privateSpotFields.map((field, index) => (
                    <div
                      key={field.id}
                      className="flex flex-col sm:flex-row gap-4 items-start sm:items-center p-4 border border-slate-200 rounded-2xl bg-slate-50"
                    >
                      <div className="flex-1 w-full">
                        ...
                        <input
                          ...
                          value={field.label}
                          onChange={(e) =>
                            updatePrivateSpotField(field.id, {
          ...
          <div className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] shadow-xl border border-white overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-slate-600 mb-6">
                Configura los campos adicionales que el vigilante debe llenar al
                registrar el ingreso de un vehículo. La placa siempre es
                obligatoria.
              </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {["hour", "hour_minute", "flat", "daily"].includes(
                        rateType,
                      ) && (
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-xl max-w-sm w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-slate-600 mb-6">
                Esta acción no se puede deshacer. La tarifa ya no estará
                disponible.
              </p>
              <div className="flex gap-3">
                <button
